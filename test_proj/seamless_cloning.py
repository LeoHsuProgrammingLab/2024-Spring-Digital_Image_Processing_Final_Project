...
import seamless_utils as utils
...
class PoissonSeamlessCloner:
    ...
    def construct_A_matrix(self):
        ...
        valid_range = (0, len(self.mask_ids) - 1)
        self.inner_pos = np.clip(self.inner_pos, *valid_range)
        self.boundary_pos = np.clip(self.boundary_pos, *valid_range)
        n1_pos = np.clip(n1_pos, *valid_range)
        n2_pos = np.clip(n2_pos, *valid_range)
        n3_pos = np.clip(n3_pos, *valid_range)
        n4_pos = np.clip(n4_pos, *valid_range)

        # print(A.shape)
        # print(max(self.inner_pos), min(self.inner_pos))
        # print(max(self.boundary_pos), min(self.boundary_pos))
        # print(max(n1_pos), min(n1_pos))
        # print(max(n2_pos), min(n2_pos))
        # print(max(n3_pos), min(n3_pos))
        # print(max(n4_pos), min(n4_pos))    
    ...
    def change_default_with_mask(self, mask):
        self.mask = mask
        self.img_h, self.img_w = self.mask.shape
        ...
        self.inner_ids = utils.get_masked_values(self.pixel_ids, self.inner_mask).flatten()
        self.boundary_ids = utils.get_masked_values(self.pixel_ids, self.boundary_mask).flatten()
        self.mask_ids = utils.get_masked_values(self.pixel_ids, mask).flatten() # boundary + inner
    ...
    #@profile
    def poisson_blend_rgb_with_input(self, src_rgb, target_rgb, mask, gradient_mixing_mode, gradient_mixing_alpha):
        self.change_default_with_mask(mask)
        self.A = self.construct_A_matrix()
        poisson_blended_img_rgb = []
        for i in range(src_rgb.shape[-1]):
            poisson_blended_img_rgb.append(
                self.poisson_blend_channel(
                    src_rgb[..., i], target_rgb[..., i],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str, default='data/test1', help="Folder of mask, source, and target image files.")
    parser.add_argument("--scale", type=float, default=1.0, help="Scaling image height and width.")
    parser.add_argument("--grayscale", action="store_true", help="Convert input to grayscale images.", default=None)
